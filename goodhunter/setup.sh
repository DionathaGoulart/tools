#!/usr/bin/env bash
# setup.sh — put goodhunter/ on PATH (macOS / Linux / Windows via Git Bash)
#
# Two ways to use it:
...
if [ "$SOURCED" -eq 1 ]; then
  ...
  if grep -qsF "goodhunter/setup.sh" "$RC"; then
    echo "goodhunter: already installed in $RC"
  else
    printf '\n# tools/goodhunter — deteccao de software de monitoramento\n%s\n' "$LINE" >> "$RC"
    echo "goodhunter: installed in $RC"
  fi
  echo "goodhunter: open a new terminal or run:  source ${RC/#$HOME/~}"
fi
