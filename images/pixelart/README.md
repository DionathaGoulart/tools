...
```bash
pixelart foto.jpg                     # 128px de grade, 16 cores, tamanho ~original
pixelart logo.svg -w 64 -c 8          # grade menor, paleta menor
pixelart *.png --palette gameboy      # lote, paleta clássica de 4 verdes
pixelart arte.png -w 48 -d            # cores chapadas → com dithering
pixelart icon.svg -w 32 --true        # salva no tamanho real da grade (1x1)
```
...
`PIXELART_TEMA` ou `RETRO_TEMA`; veja `pixelart --temas`. No terminal com truecolor
