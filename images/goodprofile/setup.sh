#!/usr/bin/env bash
# setup.sh — put images/goodprofile/ on PATH (macOS / Linux / Windows via Git Bash)
#
# Two ways to use it:
...
if [ "$SOURCED" -eq 1 ]; then
  ...
  if grep -qsF "images/goodprofile/setup.sh" "$RC"; then
    echo "goodprofile: already installed in $RC"
  else
    printf '\n# tools/images/goodprofile — comando goodprofile (imagem -> tamanhos de perfil)\n%s\n' "$LINE" >> "$RC"
    echo "goodprofile: installed in $RC"
  fi
  echo "goodprofile: open a new terminal or run:  source ${RC/#$HOME/~}"
fi
