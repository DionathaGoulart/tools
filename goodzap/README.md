...
```bash
goodzap conversa.txt                       # só as estatísticas (100% offline)
goodzap conversa.txt --roast               # + resumo, frases icônicas e roast (LLM)
goodzap conversa.txt --roast --reais       # roast com os nomes reais (menos privado)
goodzap conversa.txt --html retro.html     # também gera um retrô visual em HTML
goodzap -m                                 # lista modelos :free disponíveis
goodzap temas                              # lista as paletas do terminal
```

## Como exportar a conversa

No WhatsApp: abra a conversa → menu → **Exportar conversa** → **Sem mídia**.
Você recebe um `.txt`. É esse arquivo que o `goodzap` lê. Funciona com export
...
O `goodzap` segue o style guide de terminal retrô do repositório
(`.harness/styleguide-terminal.md`): janela de terminal, barras `█`/`▒` em
tom de acento, rótulos em maiúsculas, `[ OK ]` / `[ ERRO ]`.

```bash
goodzap temas                 # mostra as 9 paletas, marcando a atual com ►
export GOODZAP_TEMA=cyber-teal    # só o goodzap
...
| `GOODZAP_TEMA` | paleta do terminal (veja `goodzap temas`) | `vault-gold` |
