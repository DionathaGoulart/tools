...
```bash
goodprof "DNS"        # nova sessão sobre um assunto
goodprof -r           # revisão: sabatina sobre o tópico mais esquecido
goodprof -r "DNS"     # revisão de um tópico específico
goodprof -l           # lista tópicos já estudados
goodprof -m           # lista modelos :free disponíveis agora
goodprof temas        # paletas disponíveis
```

## Como funciona uma sessão

1. `goodprof "TCP handshake"` — você digita sua explicação (várias linhas;
...
Abra um terminal novo e rode `goodprof "qualquer coisa"`.
...
| `GOODPROF_DIR` | onde salvar as sessões | `~/.professor` |
| `GOODPROF_TEMA` | paleta (veja `goodprof temas`) | `vault-gold` |
...
tenta uma lista em ordem; se todos falharem, rode `goodprof -m` pra ver os que
estão no ar e ajuste `OPENROUTER_MODEL`.

## Temas

Nove paletas vindas do style guide (`goodprof temas` lista com swatch):
...
```bash
export GOODPROF_TEMA=cyber-teal   # só o goodprof
export RETRO_TEMA=cyber-teal       # todas as ferramentas
```

`GOODPROF_TEMA` tem prioridade sobre `RETRO_TEMA`. Cores true-color (24-bit)
