#!/usr/bin/env bash
# setup.sh — put games/goodpet/ on PATH (macOS / Linux / Windows via Git Bash)
#
# Two ways to use it:
...
if [ "$SOURCED" -eq 1 ]; then
  ...
  if grep -qsF "games/goodpet/setup.sh" "$RC"; then
    echo "goodpet: already installed in $RC"
  else
    printf '\n# tools/games/goodpet — comando goodpet (tamagotchi de terminal)\n%s\n' "$LINE" >> "$RC"
    echo "goodpet: installed in $RC"
  fi
  echo "goodpet: open a new terminal or run:  source ${RC/#$HOME/~}"
fi
