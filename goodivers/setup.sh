...
if [ "$SOURCED" -eq 1 ]; then
  ...
  if ! MSYS=winsymlinks:nativestrict ln -sfn "$TOOLS_DIR/skill" "$HOME/.claude/skills/goodivers" 2>/dev/null; then
    rm -rf "$HOME/.claude/skills/goodivers"
    cp -R "$TOOLS_DIR/skill" "$HOME/.claude/skills/goodivers"
  fi
