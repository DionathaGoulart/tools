#!/usr/bin/env bash
# setup.sh — put goodcheats/ on PATH (macOS / Linux / Windows via Git Bash)
#
# Two ways to use it:
...
if [ "$SOURCED" -eq 1 ]; then
  ...
  if grep -qsF "goodcheats/setup.sh" "$RC"; then
    echo "goodcheats: already installed in $RC"
  else
    printf '\n# tools/goodcheats — comandos good, goodcheat…\n%s\n' "$LINE" >> "$RC"
    echo "goodcheats: installed in $RC"
  fi
  echo "goodcheats: open a new terminal or run:  source ${RC/#$HOME/~}"
fi
